...
<html>
  <head>
	...
	$(function(){
		...
		$("#delete").click(function(){
				...
				location.href="sm/dele.do?ids="+ids.join(",");
			});
	});
  ...
  <body>
   <center>
	...
	<form action="sm/search.do" method="post" enctype="multipart/form-data">
	   <center>
		...
		<tr>
			<td><input type="checkbox" name="chkOne" value="${lst.loginId }"></td>
			<td><%=i++ %></td>
			<td>${lst.userInfo.userName }</td>
			<td><fmt:formatDate value="${lst.loginTime}" pattern="yy-MM-dd hh:mm:ss"/></td>
			<td></td>
			<td>${lst.loginUserIp }</td>
			<c:if test="${lst.ifSuccess==0 }">
			<td>是</td>
			</c:if>
			<c:if test="${lst.ifSuccess!=0 }">
			<td>否</td>
